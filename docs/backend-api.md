...
3. **The Gateway itself.** Per the locked architecture, a small service that: authenticates devices (item 2), holds the one Hermes `API_SERVER_KEY` internally, translates the iOS-facing protocol in `docs/voice-architecture.md` into calls against Hermes' existing `/api/sessions/{id}/chat/stream`, `/api/audio/speak-stream`, `/v1/runs/{id}/stop`/`steer`, and (per the locked decision to keep STT on-device for v1) accepts already-transcribed text from the phone rather than raw audio. This is now a well-scoped adapter/proxy over known, working Hermes endpoints — not an open-ended "build conversation and voice from nothing" project. **As actually built (GW-M2/GW-M3, implemented and physically accepted 2026-08-19):** the chat proxy against `/api/sessions/{id}/chat` matches this plan; `/api/audio/speak-stream` did not — it turned out to belong to a different, undocumented desktop-internal Hermes surface than assumed here, and the Gateway ended up synthesizing voice locally via Piper instead of proxying Hermes' TTS at all (see `eve-os` `docs/voice-gateway.md`, "GW-M3 — Voice", for why). `/v1/runs/{id}/stop`/`steer` remain unused so far — barge-in is Milestone 6, not yet built.
4. **TLS/network exposure.** Settled by the locked decision (revised 2026-08-16, corrected again 2026-08-19 after physical acceptance): the Gateway is reachable at the host's home-LAN address, directly when on that LAN or through the owner's existing WireGuard VPN when away (both resolve to the same address), with TLS from a private EVE-owned CA layered on top — no Tailscale, no public CA/DNS, no public endpoint for v1 (see `docs/security.md`).

**Decided (2026-08-16): the Gateway lives in `eve-os`**, as a new service alongside the existing EVE API in the Docker Compose stack — not a separate repository. This is a deliberate extension of the currently accepted self-hosted, single-host topology (today: Postgres + EVE only, loopback-bound), so `eve-os` is updating its own architecture/roadmap docs before any Gateway implementation starts — see `eve-os/docs/voice-gateway.md`.
