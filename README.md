...
**Milestone 1 (Foundation) — done, device-verified.** A pinned `GatewayAPIClient`/`GatewayTrustEvaluator` (TLS pinned to a bundled, per-deployment EVE root CA — never the system trust store), the real three-step pairing flow (`DevicePairingService`, `PairingViewModel`), a `GatewayWebSocketClient` session-lifecycle foundation, and Keychain-backed credential storage — sideloaded onto a physical iPhone (no paid Apple Developer account needed, see `docs/sideloading.md`) and paired for real against the production `eve-os` Gateway over the owner's WireGuard VPN.

**Milestone 2 (Text conversation) — implemented, CI-verified, physical acceptance pending.** `ConversationViewModel`/`ConversationView` hold a real text conversation over the Gateway's WS session (`conversation.message` → `conversation.response`), backed by the Gateway's new `eve/gateway/hermes_client.py` proxying to Hermes' own chat API. See [`docs/roadmap.md`](docs/roadmap.md) for exactly what that does and doesn't cover, and `EVE/Resources/EVERootCA-README.md` for the one manual step required before this app can build against a real deployment.
