...
**Discovery / Milestone 0.** This repository currently contains: architecture and protocol documentation, a decision record on the client-side voice pipeline, and an Xcode project skeleton with stub services. No feature is implemented end-to-end yet, and no code in this repository has run against real hardware or a real backend. See [`docs/roadmap.md`](docs/roadmap.md) for the milestone plan and current status per milestone.

**Backend picture, updated 2026-08-16:** the EVE API in `eve-os` is a memory/identity store with a single static bearer token — no conversation, voice, streaming, or per-device authentication. Hermes owns the conversation/voice runtime; after reading Hermes' actual source ([`nousresearch/hermes-agent`](https://github.com/nousresearch/hermes-agent), confirmed to be the real system `eve-os` integrates with), it turns out Hermes already has a working streaming chat API, an interrupt/steer mechanism, and pluggable TTS/STT in production use. The architecture is now locked: a dedicated **EVE Voice Gateway** sits between this app and Hermes (`EVE iOS App → EVE Voice Gateway → Hermes/EVE`), handling device pairing, sessions, and streaming — see [`docs/architecture.md`](docs/architecture.md) for the decision and [`docs/backend-api.md`](docs/backend-api.md) for exactly what already exists versus what the Gateway still needs to build. **Milestone 1 cannot reach a real backend conversation until that Gateway exists**, but it is now a well-scoped adapter over known endpoints, not an open-ended build.
