...
4. **TLS/network exposure.** Settled by the locked decision: Tailscale + TLS layered on top, no public endpoint for v1 (see `docs/security.md`).

Where should the Gateway live? Not yet decided — plausibly a new service in `eve-os` (alongside the existing EVE API and Docker Compose stack, since backend changes default to living there) or its own repository. This is the next concrete decision to make before Milestone 1 implementation starts.
