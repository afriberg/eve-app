...
**Status: Client foundation implemented and CI-green — physical device/Gateway pairing (real WireGuard, real iPhone) not yet verified.** The Gateway itself is implemented (`eve-os` `eve/gateway/`, repository/CI-verified — see its `docs/voice-gateway.md`). Client-side: `GatewayAPIClient` (health/pairing/session REST calls), `GatewayTrustEvaluator` (pins to the bundled EVE root CA, fails closed if it's absent), `GatewayWebSocketClient` (WSS session-lifecycle foundation), `DevicePairingService` + `PairingViewModel` (the real three-step request → owner-approve → claim flow, with a bounded poll loop), wired into Settings. This environment has no Xcode/macOS toolchain, so verification ran on GitHub Actions (macOS runner): `xcodebuild build` and `xcodebuild test` both green as of commit `fd67472` — including `EVEUITests.testAppLaunchesToVoiceScreen` passing on a real booted iOS Simulator (app launches, shows the "EVE" voice screen) and the full `EVETests` unit suite (`GatewayAPIClientTests`, `DevicePairingServiceTests`, `PairingViewModelTests`, `GatewayTrustEvaluatorTests`, plus the Milestone-0 tests) passing against a mocked `URLProtocol`. One real bug was caught and fixed this way: a missing `project.yml` `GENERATE_INFOPLIST_FILE` setting on the test targets (code-signing failure, unrelated to app logic) and a test-helper bug (a test client left unconfigured, so it correctly threw `notConfigured` rather than exercising the mock). Simulator CI does not cover microphone/Siri/Action Button/real Gateway network reachability — that remains physical-device work. See `docs/backend-api.md` for exactly what's real vs. still foundation-only.
