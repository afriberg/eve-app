...
**Open decision, deliberately not resolved here:** whether to target `SpeechAnalyzer`/`SpeechTranscriber` (iOS 26+ only, newer/better) or fall back to `SFSpeechRecognizer` for broader OS-version support, and exact on-device Swedish (`sv-SE`) coverage for whichever is chosen. Apple does not publish a stable, versioned locale table for either API; this must be checked programmatically against the actual deployment target during Milestone 0/2 implementation, not assumed from documentation, and re-checked on every OS version bump this app supports.
