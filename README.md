...
**Important backend gap:** the EVE API in `eve-os` today is a memory/identity store with a single static bearer token — it has no conversation, voice, streaming, or per-device authentication endpoints. Hermes owns the conversation/voice runtime, but Hermes' own API is a separate, external system not present in the `eve-os` repository and not yet documented for this purpose. Concretely, **Milestone 1 cannot reach a real backend conversation until that Hermes-side (or a new dedicated gateway) surface exists** — see [`docs/backend-api.md`](docs/backend-api.md) for the precise list of what's missing and where it should be built.
